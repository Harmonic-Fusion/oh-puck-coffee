...
export async function POST(request: NextRequest) {
  ...
  const parsed = myGrinderCollectionBodySchema.safeParse(body);
  if (!parsed.success) {
  ...
  if (parsed.data.grinderId) {
    const grinderId = parsed.data.grinderId;
    const [row] = await db
      .select({
        id: equipment.id,
        name: equipment.name,
      ...
      .where(and(eq(equipment.id, grinderId), eq(equipment.type, GRINDER)))
      .limit(1);

    if (!row) {
      return NextResponse.json({ error: "Grinder not found" }, { status: 404 });
    }

    const visible =
      row.isGlobal || row.createdBy === userId;
    if (!visible) {
  ...
  if (nameTaken) {
    return NextResponse.json(
      { error: "A grinder with this name already exists" },
      { status: 409 },
    );
  }
  ...
  if (parsed.data.specs !== undefined) {
    const v = validateSpecsForEquipmentType("grinder", parsed.data.specs);
    if (!v.ok) {
  ...
  if (grinder.imageId) {
    const [imgRow] = await db
      ...
      .from(images)
      .where(eq(images.id, grinder.imageId))
      .limit(1);
