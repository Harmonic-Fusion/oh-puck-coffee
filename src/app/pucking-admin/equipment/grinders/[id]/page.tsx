...
export default function AdminGrinderDetailPage({
  params,
}: {
  ...
  const { data, isLoading, isError } = useQuery<GrinderDetail>({
    queryKey: ["admin-grinder-detail", id],
    queryFn: async () => {
      const res = await fetch(`/api/admin/equipment/grinders/${id}`);
      if (!res.ok) throw new Error("Failed to fetch grinder details");
      return res.json();
    },
  });
  ...
  return (
    <div>
      <AdminBreadcrumb
        segments={[
          { label: "Grinders", href: equipment.grinders.path },
          { label: isLoading ? "..." : data?.name ?? "Grinder" },
        ]}
      />
      ...
      {isError && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-center text-red-600 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400">
          Failed to load grinder details.
        </div>
      )}
