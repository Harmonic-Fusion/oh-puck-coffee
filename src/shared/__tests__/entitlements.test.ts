...
describe("Entitlements", () => {
  it("exposes expected keys", () => {
    ...
    expect(Entitlements.BEAN_SHARE).toBe("bean-share");
  });
});
...
describe("hasEntitlement", () => {
  it("returns true when entitlements array contains the key", () => {
    ...
    expect(
      hasEntitlement(["bean-share"], Entitlements.BEAN_SHARE),
    ).toBe(true);
  });

  it("returns false when entitlements array does not contain the key", () => {
    expect(
      hasEntitlement(["no-shot-view-limit"], Entitlements.BEAN_SHARE),
    ).toBe(false);
    expect(hasEntitlement([], Entitlements.BEAN_SHARE)).toBe(false);
  });

  it("returns false when entitlements is undefined", () => {
    expect(
      hasEntitlement(undefined, Entitlements.BEAN_SHARE),
    ).toBe(false);
  });

  it("returns false when entitlements is null (treated like undefined via ??)", () => {
    expect(
      hasEntitlement(null as unknown as string[], Entitlements.BEAN_SHARE),
    ).toBe(false);
  });

  it("matches exact key (no partial match)", () => {
    expect(
      hasEntitlement(["bean-share-extra"], Entitlements.BEAN_SHARE),
    ).toBe(false);
    expect(
      hasEntitlement(["bean-share"], Entitlements.BEAN_SHARE),
    ).toBe(true);
...
describe("FreeEntitlementDefaults", () => {
  it("includes BEAN_SHARE so free users can share beans", () => {
    expect(FreeEntitlementDefaults).toContain(Entitlements.BEAN_SHARE);
  });
